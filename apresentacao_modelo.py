...
def encontrar_melhor_resposta(pergunta_usuario, tipo_pergunta):
    ...
    if match_fanuc:
        ...
        for falha in db_falhas:
            if falha['codigo'].upper() == codigo_falha:
                ...
                full_response = f"**Com base na sua pergunta, encontrei a melhor correspondência:**\n\n> *{melhor_pergunta}*\n\n**Resposta:**\n\n{melhor_resposta}"

                return melhor_pergunta, full_response, None, fonte, procedimento_pdf
    ...
    if detectar_palavras_oleo_lubrificante(pergunta_usuario):
        ...
        if tipo_pergunta == "FANUC":
            ...
            if melhor_score < 0.6:
                ...
                if len(db_perguntas_fanuc) < 3:
                    return None, "No momento não temos perguntas suficientes para essa consulta em nosso banco de dados FANUC.", None, None, None
                
                # Ordena os índices do maior para o menor score
                indices_ordenados = np.argsort(cos_scores.cpu().numpy())[::-1]

                # Pega as 3 melhores perguntas
                melhor_pergunta = db_perguntas_fanuc[indices_ordenados[0]]
                segunda_melhor_pergunta = db_perguntas_fanuc[indices_ordenados[1]]
                terceira_melhor_pergunta = db_perguntas_fanuc[indices_ordenados[2]]

                resposta = f"Não conseguimos encontrar um match da sua pergunta com o banco de dados, mas temos essas perguntas semelhantes:\n1. {melhor_pergunta}\n2. {segunda_melhor_pergunta}\n3. {terceira_melhor_pergunta}"
                return None, resposta, None, None, None
            ...
            if melhor_score < 0.6:
                # Verifica se há perguntas suficientes no banco
                if len(db_perguntas_abb) < 3:
                    return None, "No momento não temos perguntas suficientes para essa consulta em nosso banco de dados ABB.", None, None, None
                
                # Ordena os índices do maior para o menor score
                indices_ordenados = np.argsort(cos_scores.cpu().numpy())[::-1]

                # Pega as 3 melhores perguntas
                melhor_pergunta = db_perguntas_abb[indices_ordenados[0]]
                segunda_melhor_pergunta = db_perguntas_abb[indices_ordenados[1]]
                terceira_melhor_pergunta = db_perguntas_abb[indices_ordenados[2]]
            ...
            melhor_score = cos_scores[melhor_indice].item()
            if melhor_score < 0.6: #Se não achar boas perguntas, então mostramos as 3 melhores rankeadas
                # Verifica se há perguntas suficientes no banco
                if len(db_perguntas) < 3:
                    return None, "No momento não temos perguntas suficientes para essa consulta em nosso banco de dados.", None, None, None
                
                # Ordena os índices do maior para o menor score
                indices_ordenados = np.argsort(cos_scores.cpu().numpy())[::-1]

                # Pega as 3 melhores perguntas
                melhor_pergunta = db_perguntas[indices_ordenados[0]]
                segunda_melhor_pergunta = db_perguntas[indices_ordenados[1]]
                terceira_melhor_pergunta = db_perguntas[indices_ordenados[2]]
    ...
    full_response = f"**Com base na sua pergunta, encontrei a melhor correspondência:**\n\n> *{melhor_pergunta}*\n\n**Resposta:**\n\n{melhor_resposta}"
    return melhor_pergunta, full_response, caminho_imagem, fonte, caminho_pdf_procedimento